...
import com.rzamau.currencyconverter.data.models.Rates
...
class MainViewModel @Inject constructor(
    ...
    fun convert(
        ...
        viewModelScope.launch(dispatcher.io) {
            ...
            when (val ratesResponse = repository.getRates(base = fromCurrency)) {
                is Resource.Error -> _conversion.value =
                    CurrencyEvent.Failure(ratesResponse.message!!)
                is Resource.Success -> {
                    val rates = ratesResponse.data!!.rates
                    Log.d("TEST", "convert: $rates")
                    val rate = getRateForCurrency(toCurrency, rates)
                    if (rate == null) {
                        _conversion.value = CurrencyEvent.Failure("Unexptected Error")
                    } else {
